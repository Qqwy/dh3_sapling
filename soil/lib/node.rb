...
module Soil
	class Node < Rack::RPC::Server
		...
		def initialize(config_location, known_addresses=[])
			...
			@data_store = HashTable.new("soil_data/#{self.node_id}/data_store") #Value Store, keys are hash digests of the values.
			...
			@bucket_list = Soil::BucketList.new(self.node_id, "soil_data/#{self.node_id}/bucket_list.yml", {max_bucket_size:@@k})

			known_addresses.each do |address|
				self.ping Soil::Contact.new("", address)	
			end


			#@server = SoilServer.new(self, @address)
		...
		def read_config(config_location)
			...
			if config[:public_key].nil? && config[:signature].nil? && config[:bcrypt_salt].nil? && config[:node_id].nil? 
				...
				throw Soil::CorruptedConfigError
			elsif !Soil::Contact.valid_node_id?(config[:address], config[:public_key], config[:signature], config[:bcrypt_salt], config[:node_id])
				throw Soil::CorruptedConfigError
			else
		...
		def create_node_id(config)
			...
			if config[:address].nil?
				throw Soil::NoAddressInConfigError
			end
			...
			digest = Soil.digest_class.digest(address)
			signature_point = nil
			...
			bcrypt_salt = BCrypt::Engine.generate_salt(Soil.bcrypt_salt_strengh)

			bcrypted_signature = BCrypt::Engine.hash_secret(signature, bcrypt_salt)

			node_id = Soil.digest_class.digest(bcrypted_signature)
		...
		def to_contact
			Soil::Contact.new(@node_id, @address, @public_key, @signature, @bcrypt_salt)
		end
		...
		def handle_store(key, value)
			#key = Soil.digest_class.digest value
			actual_key = @data_store.store(key, value)
			@logger.info "Storing `#{actual_key} `locally"
			return actual_key
		end

		#Primitive operation to require contact to return up to @@k Soil::Contacts closest to given key
		def find_node(contact, key_hash)
			...
			begin
				contact.client do |c| 
					...
					result = hashed_contacts.map{|hashed_contact| Soil::Contact.from_hash(hashed_contact)}
					return result
				end
			rescue Soil::ClientConnectionError
				...
				begin
					if use_find_value then
						...
						if result["found"] then
							...
							new_shortlist = result["closest_nodes"].map{|hashed_contact| Soil::Contact.from_hash(hashed_contact)}
							@logger.info new_shortlist
						end
					else
						new_shortlist = find_node(contact, key_hash)
					end
				rescue Soil::ClientConnectionError => e
		...
		def add_or_update_contact(contact_info, i_was_called:false)
			contact = Soil::Contact.from_hash(contact_info)
