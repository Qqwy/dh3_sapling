...
module Soil
	class Contact
		...
		def self.valid_node_id?(address, public_key, signature, bcrypt_salt, node_id)
			...
			return false if BCrypt::Engine.autodetect_cost(bcrypt_salt) < Soil.bcrypt_salt_strengh

			test_node_id = Soil.digest_class.digest(BCrypt::Engine.hash_secret(signature, bcrypt_salt))
		...
		def client
			begin 
				uri = URI(@address)
				@client ||= Soil::Client.new(uri.host, uri.port, path: uri.path.empty? ? "/" : uri.path) #Only initialize once. Re-use while Contact exists and program keeps running.
				yield @client 
				...
				@can_be_called = true
	       		...
	       		raise Soil::ClientConnectionError
	   		end
	   		return true
		end
