module Soil
	class NodeMiddleware
		...
		def initialize(app, kademlia_configfile_path, known_addresses, options={})
			...
			@node = Soil::Node.new(Pathname.new(kademlia_configfile_path), known_addresses)
			@server = Rack::RPC::Endpoint.new @app, @node, path: "/"
		end

		def call(env)
			env['soil_node'] = @node #Add handle to node to the env stack, so Rack apps down the line can access it.
			@server.call(env)
		end
	end
end