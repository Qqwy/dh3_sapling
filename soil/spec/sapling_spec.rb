...
describe SoilContact do 

	before :each do 
		@kc = SoilContact.new("contact1", "127.0.0.1", "3001")
	end	

	describe "@node_id" do 
		it "should hash the passed identifier and keep it as node_id" do 
			expect(@kc.node_id).to eq (Soil.digest_class.digest "contact1")
		end
	end
	describe "@custom_time" do
		it "should keep track of the last contact_time if created with a different time" do 
			...
			@kc2 = SoilContact.new("contact2", "127.0.0.1", "3002", contact_time: custom_time)
	...
	describe "#==" do
		...
		it "should equal another contact with the same settings" do 
			@kc2 = SoilContact.new("contact1", "127.0.0.1", "3001")
			expect(@kc == @kc2).to be true
		end
		it "should not be equal to  a contact with different settings." do
			@kc2 = SoilContact.new("contact1", "127.0.0.1", "3002")
	...
	describe "#to_hash and #from_hash" do
		it "self.to_hash.from_hash should equal self" do
			expect(SoilContact.from_hash(@kc.to_hash) == @kc).to be true
		end
	end
end


describe SoilBucket do
	before :each do 
		@kb = SoilBucket.new(100,2**256, {max_bucket_size:2})
		@kc = SoilContact.new("contact1", "127.0.0.1", "3001")
	end
...
describe SoilBucketList do
	#TODO
end

describe SoilNode do
	before :each do 
		@kn = SoilNode.new("test_identifier", [
			SoilContact.new("contact1", "127.0.0.1", "3001"),
			SoilContact.new("contact2", "127.0.0.1", "3002")
			])
	end

	describe "#new" do 
		it "takes at least one parameter and returns a SoilNode" do 
			expect(@kn).to be_an_instance_of SoilNode
		end
	end
	...
	describe "#save_closest_contact" do 
		it "should return an empty array when passed an empty array" do 
			expect(@kn.save_closest_contact([], Soil.digest_class.digest("100")).empty?).to be true
		end
		it "should return an array [closest contact, distance to this contact], when being passed multiple" do 
			closest_contact = SoilContact.new("102", "127.0.0.1", "3004")
			close_contacts = [
					closest_contact,
					SoilContact.new("101", "127.0.0.1", "3005"),
					SoilContact.new("103", "127.0.0.1", "3006")
				]
			result = @kn.save_closest_contact(close_contacts, Soil.digest_class.digest("100"))
			expect(result.length).to eq 2
			expect(result.first).to eq closest_contact
			expect(result.last).to eq @kn.calc_distance(Soil.digest_class.digest("100"),Soil.digest_class.digest("102"))
		end
	end
end