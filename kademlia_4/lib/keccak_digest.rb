...
require 'base64'
	...
	class KeccakDigest < Sapling::Digest
		...
		def self.digest(key)
			return Sapling::UrlsafePaddinglessBase64.encode(Digest::SHA3.digest(key, self.hash_length))
		end
		...
		def self.to_num(hash_as_str)
			Sapling::UrlsafePaddinglessBase64.decode(hash_as_str).unpack('C*').inject(0) {|a,e| a<<=8;a+=e}
		end
